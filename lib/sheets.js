...
class Spreadsheet {
  ...
  /**
   * Append multiple rows in one Sheets API request.
   * Values are plain JS values (strings, numbers, booleans, Dates).
   */
  async appendRows(sheetName, rows) {
    if (!rows || !rows.length) return;
    const serialized = rows.map(values =>
      values.map(v =>
        v instanceof Date ? v.toISOString() :
        v === null || v === undefined ? '' : v
      )
...
async ensureSheet(sheetName) {
  const cacheKey = `sheet_exists:${this.id}:${sheetName}`;
  if (await cache.get(cacheKey)) return; // already verified — skip API call
...
export function getSpreadsheetById(id) {
...
class SheetProxy {
  ...
  appendRows(rows)                         { return this._ss.appendRows(this._name, rows); }
